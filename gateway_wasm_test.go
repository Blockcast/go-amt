...
func TestDecapsulateMulticastDataRejectsMalformedIP(t *testing.T) {
	gw := &GatewayWASM{GroupAddr: net.ParseIP("232.1.2.3")}

	// A malformed packet is input from the relay. It must be rejected as an
	// error, not panic the WASM module through a failed type assertion.
	if _, _, err := gw.decapsulateMulticastData([]byte{byte(m.MulticastDataType), 0, 0x60}); err == nil {
		t.Fatal("expected malformed AMT data to be rejected")
	}

	truncatedIPv4 := append([]byte{byte(m.MulticastDataType), 0, 0x45}, make([]byte, 19)...)
	if _, _, err := gw.decapsulateMulticastData(truncatedIPv4); err == nil {
		t.Fatal("expected truncated IPv4 data to be rejected")
	}
}
